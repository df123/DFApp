...
6. **分词统计页面** - 专门的分词统计页面，支持查看全部分词统计、搜索和精确筛选
7. **快速下载** - 在分词详情页面直接下载RSS条目链接到Aria2
...
拆分到独立的 SQLite 数据库文件 **Transient.db**（与主库同目录，存放可抛弃、可再生的
非持久数据，后续同类数据也放这里），`AppRssSource`（源配置，有价值）仍留在主库。

**实现要点**：
- 连接串：`ConnectionStrings:Transient`（默认 `Data Source=./Transient.db;`），见 appsettings.json
- 建表方式：应用启动时 `TransientDbContext.EnsureTablesCreated()` 在新库执行与主库
...
- 仓储切换：Program.cs 中为 `ISqlSugarRepository<RssMirrorItem, long>` 和
  `ISqlSugarRepository<RssWordSegment, long>` 显式注册指向独立库的构造泛型仓储
  （优先于开放泛型注册），业务服务代码零改动
- 作用域连接：`TransientDbConnection`（Scoped）保证同一请求/Job 作用域内的仓储与事务
  共享同一个独立库 client
- 抓取事务：`RssMirrorFetchJob` 中 HTTP 抓取/解析在事务外执行，独立库事务只包裹
...
主库文件体积随即缩小，备份恢复轻量。Transient.db 无需备份，可随时删除重建。
...
- `skipCount`: 跳过条数（分页）
- `maxResultCount`: 最大返回条数（分页）
- `sorting`: 排序（例如：`"totalCount desc"`）
- `filter`: 过滤关键词（搜索分词，模糊匹配）
- `rssSourceId`: RSS源ID筛选（可选）
- `languageType`: 语言类型（0=中文, 1=英文, 2=日文，可选）
- `word`: 分词文本精确匹配（可选，用于点击查看按钮时）
...
```json
{
  "ConnectionStrings": {
    ...
    "Transient": "Data Source=./Transient.db;"
  }
}
```

- `Default`：主库（RSS源配置等核心数据）
- `Transient`：可抛弃数据的独立库（当前存放 RSS 镜像条目与分词，无需备份）
...
1. **关键词搜索**（模糊匹配）:
   ...
   - 左侧显示包含该关键词的所有分词统计
   - 右侧显示包含该关键词的所有分词记录
   - 提示栏显示：🔍 正在搜索包含"xxx"的分词（左侧统计 X 个，右侧详情 X 条）

2. **语言筛选**:
   - 选择语言类型（中文/英文/日文）
   - 只显示该语言的分词
   - 提示栏显示：🌐 筛选语言：中文（左侧统计 X 个，右侧详情 X 条）

3. **精确查看**:
   - 点击左侧统计表的"查看"按钮
   - 右侧切换为精确匹配该分词
   - 提示栏显示：📌 正在查看分词："xxx"（精确匹配，共 X 条记录）
...
1. **内容分析**: 通过分词统计了解RSS源的主题分布
2. **快速定位**: 搜索特定分词，快速找到相关内容
3. **批量下载**: 在分词详情页面直接下载感兴趣的条目
...
#### 新增功能
- 专门的分词统计页面（/download-subscription/rss-word-segments）
- 左右分栏布局设计
- 全部分词统计查询（支持分页，不限制数量）
- 分词详情列表查看
- 筛选条件实时提示栏
...
#### 改进
- 分词统计API改为分页模式，支持获取全部分词
- 优化前端布局，动态计算表格高度
- 添加筛选状态提示，提升用户体验
- 支持模糊搜索和精确筛选两种模式

#### 技术细节
- RssWordSegmentAppService.GetStatisticsAsync 改为返回 PagedResultDto
- 前端使用 el-row/el-col 实现左右布局
- 使用 window resize 监听实现动态高度计算
