...
| 数据库配置令牌后_补数任务请求应携带该令牌 | 令牌只读数据库（AppConfigurationInfo） |
| 数据库配置令牌后_手动抓取请求也应携带该令牌 | 令牌只读数据库（AppConfigurationInfo） |
| 配置文件配置令牌_已移除不再生效 | 移除 appsettings/环境变量配置途径 |
...
- 代理共享密钥（`X-Proxy-Token` 请求头）**只在数据库配置**：`AppConfigurationInfo` 表中模块 `DFApp.Web.Lottery`、名称 `LotteryProxyToken`（可通过界面的配置管理修改，SQL 见 `sql/24`）；未配置或留空表示不携带令牌。值与代理端 `ProxySettings:ProxyToken` 配成对。appsettings/环境变量途径已移除
...
- `sql/24` 不再种子真实代理令牌，`sql/26` 用于将历史泄露令牌置空。
- 彩票数据页按后端 `/batch` 契约提交红球和蓝球记录数组，删除时使用期号加组号的组删除接口。
- 中奖统计前端类型补充 `buyAmount`，与 `StatisticsWinDto` 保持一致。
