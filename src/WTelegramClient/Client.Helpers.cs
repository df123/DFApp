		...
		public async Task<Storage_FileType> DownloadFileAsync(InputFileLocationBase fileLocation, Stream outputStream, int dc_id = 0, long fileSize = 0, ProgressCallback progress = null)
		{
			...
			while (!abort)
			{
				...
				async Task<int> LoadPart(long offset)
				{
                    ...
                    catch (RpcException ex) when (ex.Code == 420 && ex.Message.Contains("_WAIT_"))
                    {
						Helpers.Log(4,$"Test_DownloadFileAsync{ex.Message}");
                        // Extract numeric value from message like "FLOOD_PREMIUM_WAIT_9"
                        var waitStr = ex.Message.Split('_').Last(); // Get last part after _
                        if (int.TryParse(waitStr, out int waitSeconds))
                        {
                            Helpers.Log(4, $"Test_DownloadFileAsync_Time_{waitSeconds}");
                            await Task.Delay(waitSeconds * 1000);
                        }
						else
						{
                            Helpers.Log(4, $"Test_DownloadFileAsync_Time_{20}");
