...
/// <summary>
/// 双色球模拟控制器，提供双色球模拟投注的增删改查及模拟功能
/// </summary>
[ApiController]
[Route("api/app/lottery-ssq-simulation")]
[Authorize]
public class LotterySSQSimulationController : DFAppControllerBase
{
    private readonly LotterySSQSimulationService _ssqSimulationService;

    /// <summary>
    /// 构造函数
    /// </summary>
    /// <param name="ssqSimulationService">双色球模拟服务</param>
    /// <param name="currentUser">当前用户</param>
    /// <param name="permissionChecker">权限检查器</param>
    public LotterySSQSimulationController(
        LotterySSQSimulationService ssqSimulationService,
        ICurrentUser currentUser,
        IPermissionChecker permissionChecker)
        : base(currentUser, permissionChecker)
    {
        _ssqSimulationService = ssqSimulationService;
    }
    ...
    {
        var result = await _ssqSimulationService.GetListAsync();
    ...
    {
        var result = await _ssqSimulationService.GetAsync(id);
        return Success(result);
    }
    ...
    public async Task<IActionResult> GetPagedListAsync([FromQuery] int pageIndex = 1, [FromQuery] int pageSize = 10)
    {
        ...
        var result = await _ssqSimulationService.GetPagedListAsync(skipCount, pageSize);
        return Success(result);
    }
    ...
    {
        var result = await _ssqSimulationService.CreateAsync(input);
        return Success(result);
    }
    ...
    {
        var result = await _ssqSimulationService.UpdateAsync(id, input);
        return Success(result);
    }
    ...
    {
        await _ssqSimulationService.DeleteAsync(id);
        return Success();
    }
    ...
    {
        var result = await _ssqSimulationService.GenerateRandomNumbersAsync(input);
        return Success(result);
    }
    ...
    {
        var result = await _ssqSimulationService.GetStatisticsAsync();
        return Success(result);
    }
    ...
    {
        await _ssqSimulationService.DeleteByTermNumberAsync(termNumber);
        return Success();
    }
}
