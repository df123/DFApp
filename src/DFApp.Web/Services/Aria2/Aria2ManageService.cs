...
/// <summary>
/// Aria2 管理服务，直接通过 RPC 与 Aria2 交互
/// </summary>
public class Aria2ManageService : AppServiceBase
{
    ...
    private readonly ILogger<Aria2ManageService> _logger;
    ...
    public Aria2ManageService(
        ...
        ILogger<Aria2ManageService> logger)
    ...
    public async Task<Aria2GlobalStatDto> GetGlobalStatAsync()
    {
        ...
        {
            _logger.LogError(ex, "获取 Aria2 全局状态失败");
            throw;
        }
    }
    ...
    public async Task<Aria2TaskDto> GetTaskStatusAsync(string gid)
    {
        ...
        {
            _logger.LogError(ex, "获取任务状态失败: {Gid}", gid);
            throw;
        }
    }
    ...
    public async Task<Aria2TaskDetailDto> GetTaskDetailAsync(string gid)
    {
        ...
        {
            _logger.LogError(ex, "获取任务详情失败: {Gid}", gid);
            throw;
        }
    }
    ...
    public async Task<string> AddUriAsync(AddDownloadRequestDto input)
    {
        try
        {
            ...
            {
                throw new BusinessException("URL 列表不能为空");
            }
    ...
    public async Task<List<string>> BatchAddUriAsync(BatchAddUriRequestDto input)
    {
        try
        {
            ...
            foreach (var url in input.Urls)
            {
                ...
                {
                    _logger.LogError(ex, "添加 URL {Url} 失败", url);
                    // 继续处理其他 URL
                }
            }
        ...
        {
            _logger.LogError(ex, "批量添加 URI 下载任务失败");
            throw;
        }
    }
    ...
    public async Task<string> AddTorrentAsync(AddTorrentRequestDto input)
    {
        try
        {
            ...
            {
                throw new BusinessException("种子文件数据不能为空");
            }
        ...
        {
            _logger.LogError(ex, "添加种子文件下载任务失败");
            throw;
        }
    }
    ...
    public async Task<List<string>> BatchAddTorrentAsync(BatchAddTorrentRequestDto input)
    {
        try
        {
            ...
            {
                throw new BusinessException("种子文件列表不能为空");
            }
            ...
            foreach (var torrent in input.Torrents)
            {
                ...
                {
                    _logger.LogError(ex, "添加种子文件 {FileName} 失败", torrent.FileName);
                    // 继续处理其他种子文件
                }
            }
        ...
        {
            _logger.LogError(ex, "批量添加种子文件下载任务失败");
            throw;
        }
    }
    ...
    public async Task<List<string>> PauseTasksAsync(PauseTasksRequestDto input)
    {
        try
        {
            ...
            {
                throw new BusinessException("GID 列表不能为空");
            }
            ...
            foreach (var gid in input.Gids)
            {
                ...
                {
                    _logger.LogWarning(ex, "暂停任务失败: {Gid}", gid);
    ...
    public async Task<List<string>> UnpauseTasksAsync(PauseTasksRequestDto input)
    {
        try
        {
            ...
            foreach (var gid in input.Gids)
            {
                ...
                {
                    _logger.LogWarning(ex, "恢复任务失败: {Gid}", gid);
                    results.Add(gid);
                }
            }
    ...
    public async Task<List<string>> StopTasksAsync(StopTasksRequestDto input)
    {
        try
        {
            ...
            foreach (var gid in input.Gids)
            {
                ...
                {
                    _logger.LogWarning(ex, "停止任务失败: {Gid}", gid);
                    // 尝试强制停止
                    try
                    {
    ...
    public async Task<List<string>> RemoveTasksAsync(RemoveTasksRequestDto input)
    {
        try
        {
            ...
            foreach (var gid in input.Gids)
            {
                ...
                {
                    _logger.LogWarning(ex, "删除任务失败: {Gid}", gid);
                    results.Add(gid);
                }
            }
    ...
    public async Task<Aria2ConnectionStatusDto> GetConnectionStatusAsync()
    {
        ...
        {
            _logger.LogError(ex, "获取 Aria2 连接状态失败");
            return new Aria2ConnectionStatusDto
            {
                IsConnected = false,
    ...
    public async Task<List<IpGeolocationDto>> GetIpGeolocationAsync(List<string> ips)
    {
        try
        {
            ...
            {
                _logger.LogWarning("ip-api.com 返回错误状态: {StatusCode}, 响应: {Response}",
            ...
            if (results == null)
            {
                _logger.LogWarning("解析 ip-api.com 响应失败");
                return CreateErrorResults(ipsToQuery, "解析响应失败");
            }

            _logger.LogDebug("成功查询 {Count} 个IP的地理位置信息", results.Count);
            return results;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "批量查询IP地理位置信息失败");
