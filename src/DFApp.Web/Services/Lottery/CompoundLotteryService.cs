...
public class CompoundLotteryService : AppServiceBase
{
    ...
    public CompoundLotteryService(
        ...
        ISqlSugarRepository<LotteryInfo, long> lotteryInfoRepository,
    ...
    {
        // 验证输入
        string validationError = ValidateCompoundInput(dto);
        ...
        catch (Exception ex)
        {
            ...
            throw new BusinessException("计算复式投注组合失败: " + ex.Message);
        }
    }
    ...
    /// 验证复式投注输入
    /// </summary>
    private string ValidateCompoundInput(CompoundLotteryInputDto dto)
    {
        if (dto == null)
            return "输入数据不能为空";
    ...
    private async Task<List<LotteryDto>> SaveCombinationsToDatabase(CompoundLotteryInputDto dto, List<string> combinations)
    {
        ...
        if (lotteryInfos.Any())
        {
            try
            {
                _lotteryInfoRepository.BeginTran();
                await _lotteryInfoRepository.InsertAsync(lotteryInfos);
                _lotteryInfoRepository.CommitTran();
            }
            catch (Exception)
            {
                _lotteryInfoRepository.RollbackTran();
                throw;
            }
        }
