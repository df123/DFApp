...
namespace DFApp.Background
{
    public class ListenTelegramService : BackgroundService
    {
                ...
                try
                {
                    while (await IsSpaceUpperLimit(requiredSpace))
                    {
                        ...
                        if (isLoopDownload)
                        {
                            ...
                            mediaToDelete = GetHighDensityMediaToDelete(queryable.ToList());
                        }
                        else
                        {
        ...
        private MediaInfo? GetHighDensityMediaToDelete(List<MediaInfo> mediaList)
        {
            ...
            try
            {
                timeWindowMinutes = int.Parse(GetConfigurationInfo("TimeDensityWindowMinutes").Result);
            ...
            Logger.LogInformation($"开始执行时间密度删除算法，共有 {mediaList.Count} 个媒体文件，时间窗口: {timeWindowMinutes} 分钟");

            // 按创建时间分组，计算每个时间窗口的媒体数量
            var mediaGroups = mediaList
                .Where(x => x.LastModificationTime.HasValue)
                .GroupBy(x => {
                    var time = x.LastModificationTime!.Value;
            ...
            {
                Logger.LogInformation($"时间组 {group.TimeKey.Year}-{group.TimeKey.Month:D2}-{group.TimeKey.Day:D2} {group.TimeKey.Hour:D2}:{group.StartMinute:D2}-{group.EndMinute:D2} 包含 {group.Count} 个文件");
            }
            ...
            if (mediaGroups.Count == 0 || mediaGroups.All(g => g.Count == 1))
            {
                ...
                if (oldestFile != null)
                {
                    Logger.LogInformation($"选择删除最旧的文件: {oldestFile.SavePath}, 创建时间: {oldestFile.LastModificationTime:yyyy-MM-dd HH:mm:ss}");
                }
            ...
            {
                Logger.LogInformation($"找到高密度时间组: {highDensityGroup.TimeKey.Year}-{highDensityGroup.TimeKey.Month:D2}-{highDensityGroup.TimeKey.Day:D2} {highDensityGroup.TimeKey.Hour:D2}:{highDensityGroup.StartMinute:D2}-{highDensityGroup.EndMinute:D2}, 包含 {highDensityGroup.Count} 个文件");
                ...
                {
                    Logger.LogInformation($"在高密度时间组中选择删除最旧的文件: {oldestInHighDensity.SavePath}, 创建时间: {oldestInHighDensity.LastModificationTime:yyyy-MM-dd HH:mm:ss}");
                }
            ...
            {
                Logger.LogInformation($"选择删除最旧的文件: {defaultOldestFile.SavePath}, 创建时间: {defaultOldestFile.LastModificationTime:yyyy-MM-dd HH:mm:ss}");
            }
